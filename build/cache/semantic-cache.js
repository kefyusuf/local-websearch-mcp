import { SearchIntentClassifier } from "./intent.js";
const TTL_MAP = {
...
export class SemanticCache {
    ...
    intentClassifier;
    threshold;
    constructor(embeddingProvider, vectorStore, threshold = 0.90) {
    ...
    async setCachedContent(url, content, title) {
    ...
    detectCategory(url, title) {
        ...
        if (/\b(docs|wiki|tutorial|learn|documentation|guide|stackoverflow|api)\b/.test(combined))
            return "docs";
        if (/\b(news|haber|breaking|daily|journal|gazete)\b/.test(combined))
            return "news";
        if (/\b(reddit|forum|community|discord)\b/.test(combined))
            return "social";
        if (/\b(blog|article|medium\.com|substack)\b/.test(combined))
            return "blog";
        return "general";
    }
    // --- Semantic Re-ranking ---
    async reRankResults(query, results) {
        ...
        try {
            ...
            const rankedResults = await Promise.all(results.map(async (res) => {
